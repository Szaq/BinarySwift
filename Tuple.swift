...
func applyOrder<T>(tuple: (T, T), _ bigEndian: Bool) -> (T, T) {
  return bigEndian ? (tuple.1, tuple.0) : tuple
}

func applyOrder<T>(tuple: (T, T, T, T), _ bigEndian: Bool) -> (T, T, T, T) {
  return bigEndian ? (tuple.3, tuple.2, tuple.1, tuple.0) : tuple
}

func applyOrder<T>(tuple: (T, T, T, T, T, T, T, T), _ bigEndian: Bool) -> (T, T, T, T, T, T, T, T) {
  return bigEndian ? (tuple.7, tuple.6, tuple.5, tuple.4, tuple.3, tuple.2, tuple.1, tuple.0) : tuple
}

func toUInt16(tuple: (UInt8, UInt8)) -> (UInt16, UInt16) {
  return (UInt16(tuple.0), UInt16(tuple.1))
}

func toUInt32(tuple: (UInt8, UInt8, UInt8, UInt8)) -> (UInt32, UInt32, UInt32, UInt32) {
  return (UInt32(tuple.0), UInt32(tuple.1), UInt32(tuple.2), UInt32(tuple.3))
}

func toUInt64(tuple: (UInt8, UInt8, UInt8, UInt8, UInt8, UInt8, UInt8, UInt8)) -> (UInt64, UInt64, UInt64, UInt64, UInt64, UInt64, UInt64, UInt64) {
